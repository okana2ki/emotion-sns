...
def setup_gemini():
    ...
    try:
        ...
        try:
            ...
            return client, "gemini-2.5-flash-lite: API connection successful"
            ...
            try:
                ...
                return client, "gemini-2.0-flash-lite: Using fallback model"
            except Exception as fallback_error:
                if DEBUG_MODE:
                    st.error(f"❌ フォールバックモデルも利用できません: {fallback_error}")
                return None, f"Both models failed: {primary_error}, {fallback_error}"
...
# Gemini設定とデバッグ情報
client, setup_status = setup_gemini()

# 使用中のモデル名を取得
current_model = "gemini-2.5-flash-lite" if client and "2.5" in setup_status else "gemini-2.0-flash-lite" if client else None
...
with col_status2:
    if client:
        model_display = "Gemini 2.5" if "2.5" in setup_status else "Gemini 2.0"
        st.success(f"🤖 AI分析：{model_display}")
    else:
        st.error("⚠️ 基本分析モード")
...
with st.sidebar:
    ...
    # デバッグモード切り替え（管理者用）
    admin_input = st.text_input("管理者モード", type="password", help="デバッグ機能にアクセス")
    if admin_input == st.secrets.get("admin_password", "opencampus2024"):
        ...
        if current_model == "gemini-2.5-flash-lite":
            ...
            st.caption("• 250,000 tokens/minute") 
            st.caption("• 1,000 requests/day")
        elif current_model == "gemini-2.0-flash-lite":
            st.warning("**フォールバックモデル**")
            st.caption("• 30 requests/minute")
            st.caption("• 1,000,000 tokens/minute")
            st.caption("• 200 requests/day")
    ...
    if admin_password == st.secrets.get("admin_password", "opencampus2024"):
        ...
        if st.button("🧪 AI分析テスト", use_container_width=True):
            ...
            with st.spinner("AI分析テスト中..."):
                test_result = analyze_sentiment_with_llm(test_text, client, current_model)
            st.json(test_result)
    ...
    if message and nickname and len(message.strip()) > 5 and not st.session_state.is_posting:
        ...
        with col_model:
            if client and "フォールバック" not in reason:
                model_version = "Gemini 2.5" if "2.5" in str(reason) else "Gemini 2.0"
                st.success(model_version)
            else:
                st.warning("基本分析")
